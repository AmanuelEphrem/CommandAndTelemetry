...
def recImgTest(imgLen: int):
	...
	while tAssemb <= imgLen:
		print("assemb", assembled)
		print("tassemb", tAssemb)
		print("i", i) # If we have the same number (or greater) of assembled packets as we expected, end this loop.
		if i >= 5: # This is the same at at the bottom of the loop, but this just catches it if the last packet was None
			...
			recNums = []
			locPos = []
			assembled -= bin(failed).count("1")
			continue
		packet = comm.recPacket(5) # Wait 500 for a packet
		...
		if i >= 5: # If we received 5 packets since the last time we checked
			failed = checkLoc(locPos) # Gets the number sent back to sendImg of packets that were not received. Between 0-31
